...
class TextRNN(nn.Module):
    def __init__(self, config, weights=None):
        ...
        self.lstm = nn.LSTM(config['embedding_dim'], config['hidden_size'], num_layers=2,bidirectional=True)

        self.output = nn.Linear(2 * config['hidden_size'], config['num_class'])
        self.dropout = nn.Dropout(config['dropout'])

    def forward(self, x):
        embedd_x = self.embedding(x)
        output, (h, c) = self.lstm(embedd_x)
